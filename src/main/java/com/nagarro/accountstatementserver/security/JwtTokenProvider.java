...
public class JwtTokenProvider {
    ...
    @Value("${security.jwt.token.secret-key:app-account-statement-secret-wilecoyote}")
    ...
    private LoadingCache<String, String> invalidTokenCache;
    ...
    protected void init() {
        ...
        invalidTokenCache = CacheBuilder
                .newBuilder()
    ...
    public boolean addTokenToInvalidCache(HttpServletRequest req) {
        try {
            String token = resolveToken(req);
            validateToken(token);
            String username = getUsername(token);
            invalidTokenCache.put(username, token);
            return true;
        } catch (Exception ex) {
            log.error("Exception occored while adding token to ", ex);
            return false;
        }
    }
